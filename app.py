...
def line():
    ...
    for t in range (len(observations)):
        ser1 = ser1 + "{ x: '" + str(observations[t][5]) + "', y: " + str(observations[t][0]) + "},"

    weather_baga_url = "http://opendata.fmi.fi/wfs?request=getFeature&storedquery_id=fmi::observations::weather::multipointcoverage&fmisid=100996&parameters=Temperature,Humidity,DewPoint,WindSpeedMS,WindDirection,WindGust"
    ...
    return render_template('line_chart.html', title='Harmaja-wind', wmax=wdmax, wmin=wdmin, t=tstamp, s=windspd, d=winddir)
